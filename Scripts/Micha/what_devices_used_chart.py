...
def extract_table_for_piechart_gviz(driver, date_str):
    ...
    components = soup.find_all("ng2-piechart-component", class_="piechart gviz")
    if not components:
        print("❌ Keine Komponente mit class 'piechart gviz' gefunden.")
        return []

    # Falls mehrere, nimm das erste (bzw. Index anpassen!)
    comp = components[0]
    table_div = comp.find("div", attrs={"aria-label": lambda x: x and "tabellarische Darstellung" in x})
    ...
    print(f"✅ {len(data)} Zeilen aus What_Devices_Used-Chart extrahiert.")
    return data
...
if __name__ == '__main__':
    ...
    csv_handler = CSVFileHandler(
        "../../Data/Scrapping data as csv/what_devices_used_chart.csv",
        headers=["Datum",
                 "Gerät",
                 "Anzahl Nutzer"
                 ]
    ...
    input("🔐 Bitte im geöffneten Fenster anmelden und zur richtigen Seite scrollen. Danach hier Enter drücken ...")
